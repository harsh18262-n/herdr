...
fn wrapped_agent_name_from_cmdline(cmdline: &str) -> Option<String> {
    ...
    // Check if the immediate parent directory of a runtime binary matches an agent
    // (e.g. /path/kiro-cli/bun → parent "kiro-cli" matches)
    for token in cmdline.split_whitespace() {
        let trimmed = token.trim_matches(|c| matches!(c, '"' | '\''));
        if let Some(parent) = std::path::Path::new(trimmed).parent() {
            if let Some(dir_name) = parent.file_name().and_then(|n| n.to_str()) {
                if let Some(agent) = parse_agent_label(dir_name) {
